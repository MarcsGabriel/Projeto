<?php
    if(!empty($_GET['id'])){
        ...
        $sqlSelect = "SELECT * FROM epi WHERE CodigoEPI=$id";
        ...
        if($result->num_rows > 0){
            ...
            {
            $NomeEPI = $user_data['NomeEPI'];
            $CodigoEPI = $user_data['CodigoEPI'];
            $QuantidadeEPI = $user_data['QuantidadeEPI'];
            $DescriçaoEPI = $user_data['DescriçaoEPI'];   
            }
...
<div class="top-bar">
    ...
    <div class="cadastroEPI">
        ...
        <form class="formulario" method="POST" action="saveEditEPI.php">
        
            <h2 class="titulo">Editar cadastro</h2>
            <label class="label">
                
                <input type="hidden" name="CodigoEPI" id="CodigoEPI" value="<?php echo$CodigoEPI ?>" class="input-bordas" placeholder="Código EPI" required="">
                <span class="focus-border"> <i></i> </span>
            
            </label>
            <label class="label">
                
                <input type="text" name="NomeEPI" id="NomeEPI" value="<?php echo$NomeEPI ?>" class="input-bordas" placeholder="Nome EPI" required="">
                <span class="focus-border"> <i></i> </span>
            
            </label>
            <label class="label">
                
                <input type="text" name="QuantidadeEPI" id="QuantidadeEPI" value="<?php echo$QuantidadeEPI ?>" class="input-bordas" placeholder="Quantidade EPI" required="">
                <span class="focus-border"> <i></i> </span>
            ...
            <label class="label">
                ...
                <input type="text" name="DescriçaoEPI" id="DescriçaoEPI" value="<?php echo$DescriçaoEPI ?>" class="input-bordas" placeholder="Descrição EPI" required="">
                <span class="focus-border"> <i></i> </span>
